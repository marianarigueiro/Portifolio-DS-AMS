# Portfólio – Desenvolvimento de Sistemas (AMS) | P-TECH / IBM

Bem-vindo(a) ao meu portfólio acadêmico e profissional do curso Desenvolvimento de Sistemas – AMS (Articulação Médio-Superior) da ETEC, realizado em parceria com a IBM através do programa P-TECH.

Aqui estão reunidos todos os projetos, atividades, documentações, trilhas e entregas desenvolvidas em 2024 e 2025, seguindo as propostas, desafios e capacitações oferecidas pela IBM.

Repositório base de referência

Este portfólio segue a mesma estrutura modelo utilizada pelo projeto da instituição:
Repositório referência:
https://github.com/etec-zona-leste-oficial/magic-notes-escola-de-musica.git

Pitch do Projeto 2024 (em Inglês):
https://drive.google.com/file/d/1hnOOVKweqULR665sYwpgxdQuqcjmObOv/view

Objetivo do Portfólio
Registrar, organizar e comprovar:

* Competências técnicas desenvolvidas no curso;
* Projetos práticos solicitados pela IBM;
* Documentações realizadas;
* Horas cumpridas nas trilhas e atividades;
* Evolução como estudante de Desenvolvimento de Sistemas.

Este repositório funciona como um portfólio completo, tanto para fins acadêmicos quanto profissionais.

🛠️ Tecnologias e Ferramentas que utilizei

Aqui você pode listar o que já usou no curso e nos projetos:

Java | Python | Kotlin | JavaScript

HTML | CSS | Bootstrap

Git e GitHub

VS Code | NetBeans | IntelliJ

Figma

MySQL / SQL

IBM SkillsBuild e trilhas P-TECH

Contato
Caso queira entrar em contato ou ver mais dos meus trabalhos:

✉️ Email: mariana.rigueiro1234@gmail.com

💼 LinkedIn: www.linkedin.com/in/mariana-de-oliveira-rigueiro-561757333
